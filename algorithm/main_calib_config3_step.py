...
from quantize.abq_llm_calib_config3_step import abqllm
...
def evaluate(lm, args, logger):
    ...
    if args.tasks != "":
        ...
        try:
            logger.info(f"{results['results']['arc_challenge']['acc'] * 100:.2f},{results['results']['arc_easy']['acc'] * 100:.2f},{results['results']['boolq']['acc'] * 100:.2f},{results['results']['hellaswag']['acc'] * 100:.2f},{results['results']['piqa']['acc'] * 100:.2f},{results['results']['winogrande']['acc'] * 100:.2f}")        
        except:
            pass
    return results
...
def main():
    ...
    parser.add_argument("--quant_map", type=str, default=None, help="Path to the quant_map file")
    
    ## ---- 新增了以下参数
    parser.add_argument("--scale_calibration", type=bool, default=True)
    parser.add_argument("--compensation_calibration", action="store_true", default=False)
    
    ## ---- 前瞻量化相关参数
    parser.add_argument("--look_ahead_layers", type=int, default=0, 
                        help="Number of layers to look ahead for calibration. 0 means traditional layer-by-layer calibration.")
    parser.add_argument("--analyze_per_layer_mse", action="store_true", default=False,
                       help="Whether to analyze per-layer MSE after calibration.")
    ...
    if args.quant_map is None:
        raise ValueError("quant_map must be provided")
...
# main_calibration.py 
# - 引入args.compensation_calibration

# quantize/abq_llm_calibration.py 
# - shift = torch.zeros_like(scale)
# - use_shift = False
# - 添加params_list的判断
# - loss函数的修改
# - from models.int_llama_layer_nomatquant import QuantLlamaDecoderLayer, QuantLlamaAttention
# - from quantize.utils_calibration

# quantize/utils_calibration.py
# - def smooth_and_quant_temporary中删除smooth_q_k_temporary部分

# Todo:
# - quantize/abq_llm_calibration.py中的loss函数(余弦+mse vs. mse)
