                ...
                for j in range(args.nsamples//args.batch_size):
                    ...
                    with traincast():
                        ...
                        for layer_idx, qlayer in zip(block, qlayers):
                            if layer_idx == last_layer_idx:
                                # 最后一层获取注意力输出
                                quant_out, attn_out = qlayer(temp_quant, attention_mask=attention_mask_batch, position_ids=position_ids, output_attentions=True)
                            else:
                                temp_quant = qlayer(temp_quant, attention_mask=attention_mask_batch, position_ids=position_ids)[0]
