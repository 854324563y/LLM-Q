...
def extract_bitwidth_from_log(log_file: str) -> Optional[Tuple[float, float, float]]:
    """从日志文件中提取weight和activation位宽数据以及bitops信息"""
    try:
        with open(log_file, 'r') as f:
            ...
            for line in reversed(lines):
                ...
                # 匹配bitops信息
                b_match = re.search(r"Total network bitops: ([\d.]+)G", line)
                if b_match and bitops is None:
                    bitops = float(b_match.group(1))
                
                # 如果所有值都找到了，就可以返回了
                if weight_width is not None and activation_width is not None and bitops is not None:
                    return (weight_width, activation_width, bitops)
...
def main(base_path: str):
    """主函数"""
    # 创建CSV表头
    print("net,factor,weight_width,activation_width,bitops")
    ...
    for dirname in os.listdir(base_path):
        ...
        for log_file in log_files:
            ...
            if result is not None:
                weight_width, activation_width, bitops = result
                # 输出CSV格式数据
                print(f"{net},{factor},{weight_width:.2f},{activation_width:.2f},{bitops:.2f}")
