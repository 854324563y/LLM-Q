            ...
            for name,module in qlayer.named_modules():
                if isinstance(module, QuantLinear):
                    ...
                    if args.compensation_calibration and ('down_proj' in name or 'fc2' in name) and (i<=2 or i >= len(layers)-4): 
                        ...
                        compensation_left = torch.zeros(module.weight.shape[0], args.rank).to(dev)
                        compensation_right = torch.ones(args.rank, module.weight.shape[1]).to(dev)
                ...
                for j in range(args.nsamples//args.batch_size):    
                    ...
                    with traincast():
                        ...
                        if args.compensation_calibration:
                            ...
                            # 检查loss_type中是否包含"mse"
                            if "mse" in args.loss_type:
                                mse_loss = loss_func(fp_inps[index:index+args.batch_size,], quant_out)
                                mse_loss += loss_func(fp_inps_2[index:index+args.batch_size,], quant_out)
                                loss += mse_loss
                            
                            # 检查loss_type中是否包含"cos"
                            if "cos" in args.loss_type:
                            ...
                            if "kl" in args.loss_type:
                                # 双向KL散度
                                fp_log_prob = F.log_softmax(fp_inps[index:index+args.batch_size,], dim=-1)
                                quant_soft = F.softmax(quant_out, dim=-1)
                                loss += torch.abs(kl_loss(fp_log_prob, quant_soft))
                                
                                quant_log_prob = F.log_softmax(quant_out, dim=-1)
                                fp_soft = F.softmax(fp_inps[index:index+args.batch_size,], dim=-1)
                                loss += torch.abs(kl_loss(quant_log_prob, fp_soft))
                                
                                # 对fp_inps_2也计算KL散度
                                fp2_log_prob = F.log_softmax(fp_inps_2[index:index+args.batch_size,], dim=-1)
                                loss += torch.abs(kl_loss(fp2_log_prob, quant_soft))
                                loss += torch.abs(kl_loss(quant_log_prob, F.softmax(fp_inps_2[index:index+args.batch_size,], dim=-1)))
                        else:
                            quant_out = qlayer(quant_inps[index:index+args.batch_size,], attention_mask=attention_mask_batch,position_ids=position_ids, output_attentions= False)[0]
