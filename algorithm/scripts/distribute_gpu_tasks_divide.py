...
def run_experiment(gpu_id, bitops_factor):
    ...
    model = "llama-13b-hf"
    log_dir = f"./log-divide-adaptive-calibration/{model}_{bitops_factor}"
    quant_map = f"./log-divide-adaptive/{model}/quant_map_{model}_{bitops_factor}.pkl"
    blocks_pkl = f"./log-divide/{model}-w4a4/{model}_blocks.pkl"
    
    # 构建命令
    cmd = f"""CUDA_VISIBLE_DEVICES={gpu_id} nohup python main_calib_config3.py \
