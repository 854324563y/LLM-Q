...
class UniformAffineQuantizer(nn.Module):
    def __init__(
        ...
        if self.disable_zero_point:
            ...
            if n_bits == 2 or n_bits == 1:
                print('------------ bit balance ------------')
                self.qmax = self.qmax + 1
    ...
    def fake_quant(self, x, scale, zero_point):
        ...
        if zero_point is not None:
            x_int = round_ste((x - zero_point) / scale)
        else:
        ...
        if zero_point is not None:
            x_dequant = x_dequant.add(zero_point)
        if self.group_size:
    ...
    def per_token_dynamic_calibration(self, x):
        ...
        if self.disable_zero_point:
            ...
            self.zero_point = zero_point.clamp(min=-1e4, max=1e4)
