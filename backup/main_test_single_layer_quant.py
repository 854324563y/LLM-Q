...
from quantize.abq_llm_test_single_layer_quant import abqllm
...
def main():
    ...
    parser.add_argument("--sensitivity_test", default="layer_sensitivity_results.pt", type=str, help="save the results of sensitivity test")
    ...
    parser.add_argument("--target_layer", type=int, default=None, 
                        help="指定要量化的层号(从0开始). 如果不指定则量化所有层")
    ...
    if args.target_layer is not None:
        # 只量化指定层
        logger.info(f"=== Quantizing only layer {args.target_layer} ===")
        abqllm(lm, args, dataloader, act_scales, act_shifts, logger)
        eval_results = evaluate(lm, args, logger)
        results[args.target_layer] = eval_results
    else:
        # 逐层量化并评估
        logger.info("=== Evaluating sensitivity of each layer ===")
        num_layers = len(lm.model.model.layers if "llama" in args.net.lower() or "mixtral" in args.net.lower() 
                        else lm.model.model.decoder.layers if "opt" in args.net.lower()
                        else lm.model.transformer.h)
        ...
        for layer_idx in range(num_layers):
            ...
            # 评估结果
            eval_results = evaluate(lm, args, logger)
            results[layer_idx] = eval_results
        ...
        # 保存所有层的评估结果
        torch.save(results, os.path.join(args.output_dir, args.sensitivity_test))
        
        # 打印汇总结果
        logger.info("\n=== Layer Sensitivity Summary ===")
        for layer_idx, result in results.items():
            logger.info(f"Layer {layer_idx}: {result}")
